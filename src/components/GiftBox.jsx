  ...
  return (
    <div className="flex items-center justify-center w-full h-screen bg-gradient-to-b from-purple-100 to-pink-100">
      <div className="relative flex flex-col items-center w-full px-6">
        {/* Gift Box */}
        <div 
          className={`relative cursor-pointer transition-all duration-700 ${isOpened ? 'scale-75 opacity-80' : 'scale-100 hover:scale-110'}`}
          onClick={handleClick}
        >
          <div className="relative">
            <Gift 
              size={150} 
              className={`text-purple-600 filter drop-shadow-lg transition-all duration-700 ${isOpened ? 'transform -translate-y-24 rotate-12' : ''}`} 
            />
            
            {/* Decorative elements */}
            <div className={`absolute -top-4 -right-4 transition-all duration-700 ${isOpened ? 'animate-spin' : ''}`}>
              <Stars size={40} className="text-yellow-500" />
            </div>
            <div className={`absolute -bottom-4 -left-4 transition-all duration-700 ${isOpened ? 'animate-bounce' : ''}`}>
        ...
        {isOpened && (
          <div className="absolute transform -translate-y-40 mx-auto p-8 bg-white rounded-2xl shadow-2xl text-center z-50 animate-message-popup w-full max-w-lg">
            ...
            <p className="text-lg font-bold text-purple-800 mb-2">
              Happy <span className="text-pink-600">30th Anniversary</span>
            </p>
            <p className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
              Shri Ishwar Chandra Prajapati 
            </p>
            <p className="text-xl font-bold text-purple-600 mb-1">and</p>
            <p className="text-2xl font-bold bg-gradient-to-r from-pink-500 to-red-500 bg-clip-text text-transparent">
              Shrimati Usha Devi ji
            </p>
          </div>
        )}
            ...
            {Array.from({ length: 80 }).map((_, i) => (
              <div 
                ...
                style={{
                  ...
                  backgroundColor: ['#FF595E', '#FFCA3A', '#8AC926', '#1982C4', '#6A4C93', '#FF85EA', '#FFC75F'][i % 7],
                  borderRadius: Math.random() > 0.5 ? '50%' : '3px',
                  left: `${Math.random() * 100}%`,
                  top: `${Math.random() * 100}%`,
                  animation: `confetti-${i % 7} ${Math.random() * 3 + 2}s ease-in-out forwards`,
          ...
          <>
            {Array.from({ length: 12 }).map((_, i) => (
              <div
                ...
                style={{
                  animation: `fly-heart-${i % 6} 2s cubic-bezier(0.21, 0.98, 0.6, 0.99) forwards`,
                  animationDelay: `${Math.random() * 0.5}s`,
                  left: '50%',
                  top: '50%',
                }}
              >
                <Heart size={24 + Math.random() * 20} fill="#FF85EA" stroke="#FF85EA" />
              </div>
            ))}
          </>
        )}
      ...
      <style jsx>{`
        ...
        @keyframes confetti-4 {
          0% { transform: translate3d(0, 0, 0) rotate(0); opacity: 1; }
          100% { transform: translate3d(-50px, 100vh, 0) rotate(1080deg); opacity: 0; }
        }
        ...
        @keyframes confetti-6 {
          0% { transform: translate3d(0, 0, 0) rotate(0); opacity: 1; }
          100% { transform: translate3d(0, 100vh, 0) rotate(1440deg); opacity: 0; }
        }
        @keyframes message-popup {
          0% { opacity: 0; transform: translateY(20px) scale(0.8); }
          50% { opacity: 1; transform: translateY(-50px) scale(1.05); }
          70% { transform: translateY(-40px) scale(1); }
          100% { transform: translateY(-40px) scale(1); }
        }
        @keyframes fly-heart-0 {
          0% { transform: translate(0, 0) scale(0.5); opacity: 0.8; }
          100% { transform: translate(-120px, -100px) scale(1); opacity: 0; }
        }
        ...
        @keyframes fly-heart-2 {
          0% { transform: translate(0, 0) scale(0.5); opacity: 0.8; }
          100% { transform: translate(-150px, -30px) scale(1); opacity: 0; }
        }
